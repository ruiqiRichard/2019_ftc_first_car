 ...
 */
@Autonomous(name = "first_blue_2")
public class auto_blue_two extends LinearOpMode{
...
//    private static final float mmTargetHeight   = (6) * mmPerInch;          // the height of the center of the target image above the floor
    ...
    private static final double driveSecond = -2000;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        translationWithPID(translation, 0.6);
...
//        translationWithPID(-1000, 0.8);

        driveWithPID(-driveFirst, 1);
        rotationWithPID(angle, 0.8);
        driveWithPID(driveSecond,0.8);
    }
...
//            telemetry.update();
    ...
    public void rotationWithGyroAndPID(double targetAngle, double yRotation) {
        double power = yRotation;
        double motorPosition;
        double target = Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) + 180 + targetAngle;
        map.initDcmotor(map.leftFront);
        while (power != 0.0) {
            map.driveMecanum(0, power, 0);
            double currentAngle = Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) + 180;
            motorPosition = currentAngle / target;
            power *= map.pid_output(map.Kp_standard, map.Kd_standard, map.Ki_standard, motorPosition, 1, false);
    ...
    public void rotationWithGyro(double targetAngle, double yRotation){
        double power = yRotation;
        double initialAngle = Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) + 180;
        double target = initialAngle + targetAngle;
        double kp;
        if (yRotation < 0){
            while (teamCode){
                double currentAngle = Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) + 180;
                if (currentAngle != target){
                    kp = currentAngle / initialAngle;
                    if (currentAngle > target - 40){
                        map.driveMecanum(0,yRotation * kp,0);
                    }
                    else {
                        map.driveMecanum(0,yRotation,0);
                    }

                }

                else if (target - 5 < currentAngle  || currentAngle < target + 5)teamCode = false;
            }
        }
